...
import {
    ...
    filterSchema,
    type Conjuntion,
    type SimpleFilter as SimpleFilterT,
    type GroupFilter as GroupFilterT,
...
function getTriggerDescription({
    ...
    const manualActivationHandler = () => "Contact manually added to workflow."
    ...
    >(
        ...
        const { workflowIds = [], stepIds = [], mailboxIds = [] } = params
        const actionMap = {
            emailOpened: "opened",
        ...
        return `Contact ${actionMap[type]} email sent${
            ...
            workflowIds.length > 0
                ? ` in workflow(s): ${makeList(workflowIds)}`
                : ""
    ...
    >(
        ...
        const { workflowIds = [], stepIds = [] } = params
        const actionMap = {
        ...
        <div className="flex w-full flex-col items-start justify-start gap-4 px-4">
            ...
            {triggers && triggers.length > 0 ? (
                triggers.map((trigger, key) => (
                    <div
                        key={key}
                        className="flex w-full flex-col items-start gap-4 rounded-md bg-slate-100 p-2"
                    >
                        {getTriggerDescription(trigger)}
                    </div>
                ))
            ) : (
                <div className="flex w-full flex-col items-start gap-4 rounded-md bg-slate-100 p-2 italic">
                    No events assigned.
                </div>
            )}
        </div>
        ...
        return (
            <div
                className={cn(
                    ...
                    this.isChild ? "bg-slate-300" : "bg-slate-100",
                )}
            >
                <div>
                ...
                <div className="flex flex-1 flex-wrap items-start justify-start gap-2">
                    <span className="rounded bg-slate-700 p-1 px-2 text-xs text-white">
                        {String(this.value)}
                    </span>
                </div>
            </div>
...
class GroupFilter implements IFilter {
    ...
    render(): JSX.Element {
        return (
            <div className="flex w-full flex-1 flex-col items-start gap-2 rounded-md bg-slate-100 p-2">
