...
export function CustomEdge({
    ...
    const topLabelAngle = useMemo(
        () =>
            Math.atan(
                (topLabelY - derivativeTopLabelY) /
                    (topLabelX - derivativeTopLabelX),
            ) +
    ...
    return (
        <>
            <BaseEdge
                path={edgePath}
                markerEnd={markerEnd}
                className=" !stroke-slate-400 !stroke-2 transition-colors  group-[.selected]:!stroke-primary-700 "
            />
            ...
            <EdgeLabelRenderer>
                {amountEdges && amountEdges > 1 && topLabelLetter && (
                    <div
                        ...
                        className="nodrag nopan absolute h-fit w-auto  rounded-md bg-slate-100 p-2 text-xs font-semibold"
                    >
                        {topLabelLetter}
                    </div>
                )}
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                ...
                >
                    <button
                        ...
                        className="flex items-center gap-1  rounded-md bg-slate-100 p-2 text-xs font-semibold  transition-colors hover:text-primary-700 group-[.selected]:!bg-primary-200"
                    >
                        {isHovered ? (
                            <PencilSimple
                                weight="bold"
            ...
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>Set Workflow Delay</DialogTitle>
                    <DialogDescription>
                        Set the delay between two steps in the workflow or
                        choose to skip the delay.
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                    <form
