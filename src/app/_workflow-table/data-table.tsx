...
export function DataTable<Workflow, TValue>({
    columns,
    data,
}: DataTableProps<Workflow, TValue>) {
    //Mock Data
    ...
    const table = useReactTable<Workflow>({
        ...
        getRowId: (row) => workflowSchema.parse(row).id,
        initialState: {
            columnVisibility: {
                owner: false,
                totalCount: false,
        ...
        meta: {
            ...
            cloneWorkflow: (rowId: string) => {
                setWorkflowsMockData((oldWorkflowsMockData) => {
                    const foundWorkflow = oldWorkflowsMockData.find(
                        (workflow) => workflow.id === rowId,
                    )
                    if (foundWorkflow) {
                        foundWorkflow.id = generateId()
                        return [...oldWorkflowsMockData, foundWorkflow]
                    }

                    return oldWorkflowsMockData
                })
            },
            archiveWorkflow: (rowId: string) => {
                setWorkflowsMockData((oldWorkflowsMockData) =>
                    oldWorkflowsMockData.map((workflow) => {
                        if (rowId === workflow.id) {
                            return {
                                ...workflow,
                                status: "archived",
                            }
